...
def coarsen(nx,ny,nxc,nyc,w,wc):
    wf = np.fft.fft2(w[1:nx+2,1:ny+2])
    
    wfc = np.zeros((nxc,nyc),dtype='complex')
    
    wfc[0:int(nxc/2),0:int(nyc/2)] = wf[0:int(nxc/2),0:int(nyc/2)]
    
    wfc[int(nxc/2):,0:int(nyc/2)] = wf[int(1 + nx-nxc/2):,0:int(nyc/2)]
    
    wfc[0:int(nxc/2),int(nyc/2):] = wf[0:int(nxc/2),int(1 + ny-nyc/2):]
    
    wfc[int(nxc/2):,int(nyc/2):] = wf[int(1 + nx-nxc/2):,int(1 + ny-nyc/2):] 
    
    wfc = wfc*(nxc*nyc)/(nx*ny)
    
    wtc = np.real(np.fft.ifft2(wfc))
    
    wc[1:nxc+1,1:nyc+1] = wtc
    wc[:,nyc+1] = wc[:,1]
    wc[nxc+1,:] = wc[1,:]
...
def write_data(nx,ny,dx,dy,nxc,nyc,dxc,dyc,w,s,k):
    ...
    np.savetxt("j_fourier_"+str(k)+".csv", jc, delimiter=",")    
    np.savetxt("j_coarsen_"+str(k)+".csv", jcoarse, delimiter=",")   
...
np.savetxt("final_"+str(nd)+"_"+str(re)+".csv", w, delimiter=",")    
...
np.savetxt("energy_"+str(nd)+"_"+str(int(re))+".csv", en, delimiter=",")
...
#ax.loglog(k,fine_es[1:k.shape[0]+1], 'm', lw = 2, label = '256_4000')

plt.xlabel('$K$')
plt.ylabel('$E(K)$')
plt.legend(loc=0)
plt.ylim(1e-8,1e-1)
fig.savefig('energy_spectrum.png', bbox_inches = 'tight', pad_inches = 0)
